  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Button onClick={()=>{ setEditing({ request_type:"Risk Acceptance", related_risk_id:"", related_object_type:"Risk", related_object_id:"", title:"", justification:"", residual_risk_level:"Medium", appetite_level:"Medium", reason:"", compensating_controls:"", effective_date: new Date().toISOString().slice(0,10), expiry_date:"", status:"Draft" }); setOpen(true); }} className="bg-blue-700 hover:bg-blue-800"><Plus className="w-4 h-4 mr-2" />New Request</Button>
      </div>
      <Card className="overflow-hidden"><table className="w-full text-sm"><thead className="bg-slate-50 border-b"><tr className="text-xs uppercase tracking-wider text-slate-500"><th className="text-left py-2 px-3">Type</th><th className="text-left py-2 px-3">Title</th><th className="text-left py-2 px-3">Risk</th><th className="text-left py-2 px-3">Level</th><th className="text-left py-2 px-3">Effective</th><th className="text-left py-2 px-3">Expiry</th><th className="text-left py-2 px-3">Status</th><th></th></tr></thead><tbody>
        {list.map(a=>(<tr key={a.id} className={`border-b border-slate-100 hover:bg-slate-50 ${a.status==="Expired"?"bg-rose-50/40":""}`}><td className="py-2 px-3 text-xs">{a.request_type}</td><td className="py-2 px-3 font-medium max-w-md truncate">{a.title}</td><td className="py-2 px-3 font-mono text-xs text-blue-700">{riskMap[a.related_risk_id]?.risk_id||"—"}</td><td className="py-2 px-3"><RiskBadge level={a.residual_risk_level} /></td><td className="py-2 px-3 text-xs">{a.effective_date}</td><td className="py-2 px-3 text-xs">{a.expiry_date}</td><td className="py-2 px-3"><StatusBadge status={a.status} /></td><td className="py-2 px-3 flex gap-1">
          {a.status==="Draft" && <Button size="sm" onClick={()=>action(a.id, "submit")} className="bg-blue-700 hover:bg-blue-800">Submit</Button>}
      ...
      </div>}<DialogFooter><Button variant="outline" onClick={()=>setOpen(false)}>Cancel</Button><Button onClick={save} className="bg-blue-700 hover:bg-blue-800">Save Draft</Button></DialogFooter></DialogContent></Dialog>
    </div>
  );
}
