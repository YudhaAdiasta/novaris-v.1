          ...
          {days.map((d, i) => {
            ...
            return (
              <div key={i} className={`min-h-[90px] border rounded-md p-2 ${today ? "border-blue-400 bg-blue-50/30" : "border-slate-200 bg-white"}`}>
                <div className={`text-xs font-semibold ${today ? "text-teal-700" : "text-slate-700"}`}>{d.getDate()}</div>
                <div className="mt-1 space-y-1">
                  {evs.slice(0,3).map((e) => (
                    <Link key={e.id} to={`/risks/${e.ref_id}`} className={`block text-[10px] px-1.5 py-0.5 rounded truncate ${e.type==="Risk Review" ? "bg-teal-50 text-teal-700" : "bg-purple-50 text-purple-700"} hover:underline`}>{e.title}</Link>
                  ))}
        ...
        <table className="w-full text-sm">
          ...
          <tbody>{events.slice(0,30).map((e) => (
            <tr key={e.id} className="border-b border-slate-100 hover:bg-slate-50">
              ...
              <td className="py-2 px-3"><span className={`px-2 py-0.5 rounded text-xs ${e.type==="Risk Review" ? "bg-teal-50 text-teal-700" : "bg-purple-50 text-purple-700"}`}>{e.type}</span></td>
              <td className="py-2 px-3">{e.title}</td>
              <td className="py-2 px-3"><Link to={`/risks/${e.ref_id}`} className="text-teal-700 hover:underline text-xs">Open</Link></td>
            </tr>
          ))}</tbody>
        </table>
      </Card>
