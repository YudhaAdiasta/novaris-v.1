  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Button onClick={()=>{ setEditing({ risk_id:"", control_id:"", test_period: new Date().toISOString().slice(0,7), test_type:"Combined Test", test_procedure:"", tester:"", sample_size:0, sample_description:"", test_result:"Not Tested", findings:"", evidence_notes:"", deficiency:false, remediation_required:false, remediation_action:"", remediation_owner:"", remediation_due_date:"", status:"Draft" }); setOpen(true); }} className="bg-blue-700 hover:bg-blue-800"><Plus className="w-4 h-4 mr-2" />New Test</Button>
      </div>
      <Card className="overflow-hidden"><table className="w-full text-sm"><thead className="bg-slate-50 border-b"><tr className="text-xs uppercase tracking-wider text-slate-500"><th className="text-left py-2 px-3">Risk</th><th className="text-left py-2 px-3">Period</th><th className="text-left py-2 px-3">Type</th><th className="text-left py-2 px-3">Tester</th><th className="text-left py-2 px-3">Result</th><th className="text-left py-2 px-3">Deficiency</th><th className="text-left py-2 px-3">Status</th><th></th></tr></thead><tbody>
        {list.map(t=>(<tr key={t.id} className={`border-b border-slate-100 hover:bg-slate-50 ${t.test_result==="Failed"?"bg-rose-50/40":""}`}><td className="py-2 px-3 font-mono text-xs text-blue-700">{riskMap[t.risk_id]?.risk_id||"—"}</td><td className="py-2 px-3 text-xs">{t.test_period}</td><td className="py-2 px-3">{t.test_type}</td><td className="py-2 px-3">{t.tester}</td><td className="py-2 px-3"><span className={`px-2 py-0.5 rounded text-xs font-medium border ${t.test_result==="Passed"?"bg-emerald-50 text-emerald-700 border-emerald-200":t.test_result==="Failed"?"bg-rose-50 text-rose-700 border-rose-200":t.test_result==="Partially Passed"?"bg-amber-50 text-amber-700 border-amber-200":"bg-slate-100 text-slate-700"}`}>{t.test_result}</span></td><td className="py-2 px-3">{t.deficiency?<span className="text-rose-600 font-semibold">YES</span>:"—"}</td><td className="py-2 px-3"><StatusBadge status={t.status} /></td><td className="py-2 px-3"><Button size="sm" variant="outline" onClick={()=>{ setEditing(t); setOpen(true); }}>Edit</Button></td></tr>))}
