    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <Button variant={tab==="committees"?"default":"outline"} onClick={()=>setTab("committees")} className={tab==="committees"?"bg-teal-600 hover:bg-teal-700":""}>Committees</Button>
          <Button variant={tab==="meetings"?"default":"outline"} onClick={()=>setTab("meetings")} className={tab==="meetings"?"bg-teal-600 hover:bg-teal-700":""}>Meetings</Button>
        </div>
      </div>
      {tab === "committees" ? (
        <Card className="overflow-hidden">
          <div className="p-3 border-b flex justify-end"><Button onClick={() => { setEditC({ name:"", type:"Risk Committee", description:"", chairperson:"", secretary:"", meeting_frequency:"Monthly", status:"Active" }); setOpenC(true); }} className="bg-teal-600 hover:bg-teal-700"><Plus className="w-4 h-4 mr-1" />Add Committee</Button></div>
        ...
        <Card className="overflow-hidden">
          ...
          <table className="w-full text-sm"><thead className="bg-slate-50 border-b"><tr className="text-xs uppercase tracking-wider text-slate-500"><th className="text-left py-2 px-3">Code</th><th className="text-left py-2 px-3">Title</th><th className="text-left py-2 px-3">Date</th><th className="text-left py-2 px-3">Status</th><th></th></tr></thead><tbody>
            {meetings.map(m => (<tr key={m.id} className="border-b border-slate-100 hover:bg-slate-50"><td className="py-2 px-3 font-mono text-xs text-teal-700">{m.meeting_code}</td><td className="py-2 px-3 font-medium">{m.title}</td><td className="py-2 px-3 text-xs">{m.meeting_date} {m.start_time}</td><td className="py-2 px-3"><StatusBadge status={m.status} /></td><td className="py-2 px-3"><Button size="sm" variant="outline" onClick={()=>{ setEditM(m); setOpenM(true); }}>Open</Button></td></tr>))}
          </tbody></table>
        </Card>
      )}
      ...
      </div>}<DialogFooter><Button variant="outline" onClick={()=>setOpenC(false)}>Cancel</Button><Button onClick={saveC} className="bg-teal-600 hover:bg-teal-700">Save</Button></DialogFooter></DialogContent></Dialog>
      ...
      </div>}<DialogFooter><Button variant="outline" onClick={()=>setOpenM(false)}>Cancel</Button><Button onClick={saveM} className="bg-teal-600 hover:bg-teal-700">Save</Button></DialogFooter></DialogContent></Dialog>
    </div>
  );
}
